...
<script>
import { LEAVE_DETAIL, LEAVE_CREATE, LEAVE_UPDATE, LEAVE_SUBMIT, LEAVE_DELETE } from '@/apis/workflow'
...
export default {
  ...
  methods: {
    ...
    onSave () {  // 保存业务单
      ...
      this.$post(
        this.queryId ? LEAVE_UPDATE : LEAVE_CREATE,
        this.form,
        data => {
          this.loading = false
          this.$toast.success(data.msg)
    ...
    onSubmit (id) {   // 启动流程
      ...
      let workFlowInstance = {
        ...
        url: `${window.location.origin + window.location.pathname}/#/workflow?type=leave`,
