<template>
  <div class="addFrom">
    ...
    <egress-form :isDisabled="isDisabled" :active="active" :formData="form" ref="form">
      <div class="heights"></div>
...
<script>
import { EGRESS_DETAIL, EGRESS_CREATE, EGRESS_UPDATE, EGRESS_SUBMIT, EGRESS_DELETE } from '@/apis/workflow'
import { mapState } from 'vuex'
import EgressForm from './Form'
import { OrgPersonSelect } from '@/components'
export default {
  components: { EgressForm , OrgPersonSelect},
  props: {
    active: Number,
    queryId: String
  },
  ...
  methods: {
    ...
    onSave () {  // 保存业务单
      ...
      this.$post(
        this.queryId ? EGRESS_UPDATE : EGRESS_CREATE,
        this.form,
        data => {
          this.loading = false
          this.$toast.success(data.msg)
    ...
    onSubmit (id) {   // 启动流程
      ...
      let workFlowInstance = {
        ...
        url: `${window.location.origin + window.location.pathname}/#/workflow?type=egress`,
