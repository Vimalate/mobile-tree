    ...
    <ShList :api="listApi" :param="form" ref="list" >
      <div class="lists_content" @click="registrationFun(scope.item)" slot-scope="scope">
        <div class="radiuces">
          <div class="flexs">
          <!-- 2 显示审核未通过 -->
          <van-cell :title-style="{'width': '200px'}" :border='false' :title="`${scope.item.creatorName || scope.item.registerName }的用车申请`" />
          <div class="btnIng">
        ...
        <div class="link">
          <p class="type">
            <span>开始时间：{{ $dayjs(scope.item.startTime).format("YYYY-MM-DD HH:mm") }}</span>
          </p>
          <p class="type">
            <span>结束时间：{{ $dayjs(scope.item.endTime).format("YYYY-MM-DD HH:mm") }}</span>
          </p>
          <p class="type">
            <span>时长(小时)：{{ $dayjs(scope.item.endTime).diff(scope.item.startTime,'minute') / 60 }}</span>
          </p>
        </div>
        <div>
          <div class="btns">
...
<script>
import { VEHICLE_LIST } from '@/apis/workflow'
export default {
  props: {
    active: Number
  },
  ...
  methods: {
    registrationFun (item) {
      if (item.approStatus == 0) {
        ...
        this.$router.push({path: '/workflow', query:{id: item.id, type: 'vehicle'}})
      }
    },
    addLink () {
      this.$emit('update:active', 0)
