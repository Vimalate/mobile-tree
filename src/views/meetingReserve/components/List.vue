    ...
    <ShList :api="listApi" :param="form" ref="list" >
      <div class="lists_content" @click="registrationFun(scope.item)" slot-scope="scope">
        <div class="radiuces">
          ...
          <van-cell :title-style="{'width': '200px'}" :border='false' :title="`主题：${scope.item.conferenceTopic}`" />
          <div class="btnIng">
            <div v-if="scope.item.status == 1" class="defalults listBtns">预定中</div>
            <div v-if="scope.item.status == 2" class="defalults listBtns">已完成</div>
          </div>
        </div>
        <div class="link">
          <p class="type">
            <span>开始时间：{{ $dayjs(scope.item.conferenceStartTime).format("YYYY-MM-DD HH:mm") }}</span>
          </p>
          <p class="type">
            <span>结束时间：{{ $dayjs(scope.item.conferenceEndTime).format("YYYY-MM-DD HH:mm")}}</span>
          </p>
          <p class="type">
            <span>时长(小时)：{{ $dayjs(scope.item.conferenceEndTime).diff(scope.item.conferenceStartTime,'minute') / 60 }}</span>
          </p>
        </div>
        <div>
          <div class="btns">
...
<script>
import { MEETING_RESERVE_LIST } from '@/apis/base'
export default {
  props: {
    active: Number,
    queryId: String
  ...
  methods: {
    registrationFun (item) {
      if(this.$dayjs(item.conferenceEndTime)>new Date()){
        this.$emit('update:active', 0)
        this.$emit('update:queryId', item.id)
      }else{
        this.$router.push({path: '/meetingReserveDetails', query:{id: item.id}})
      }
    },
    onSearch () {
    ...
    addLink () {
      this.$router.push({path: '/meetingReserve'})
      this.$emit('update:active', 0)
      this.$emit('update:queryId', '')
    },
    loadList () {
