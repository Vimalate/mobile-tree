    ...
    <ShList :api="listApi" :param="form" ref="list" >
      <div class="lists_content" @click="registrationFun(scope.item)" slot-scope="scope">
        <div class="radiuces">
          <div class="flexs">
          <!-- 2 显示审核未通过 -->
          <van-cell :title-style="{'width': '200px'}" :border='false' :title="`${scope.item.name || scope.item.creatorName }的补卡申请`" />
          <div class="btnIng">
        ...
        <div class="link">
          <p class="type">
            <span>补卡时间：{{ $dayjs(scope.item.replenishCardTime).format("YYYY-MM-DD HH:mm") }}</span>
          </p>
        </div>
        <div>
          <div class="btns">
...
<script>
import { REPLENISH_CARD_PAGE } from '@/apis/workflow'
export default {
  props: {
    active: Number,
    queryId: String
  ...
  methods: {
    registrationFun (item) {
      if (item.approStatus == 0) {
        ...
        this.$router.push({path: '/workflow', query:{id: item.id, type: 'replenishCard'}})
      }
    },
    onSearch () {
