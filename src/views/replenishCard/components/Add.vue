<template>
  <div class="addFrom">
    ...
    <replenish-card-form :isDisabled="isDisabled" :formData="form" :active="active" ref="form">
      <div class="heights"></div>
...
<script>
import { REPLENISH_CARD_DETAIL, REPLENISH_CARD_CREATE, REPLENISH_CARD_UPDATE, REPLENISH_CARD_SUBMIT, REPLENISH_CARD_DELETE } from '@/apis/workflow'
import { mapState } from 'vuex'
import ReplenishCardForm from './Form'
import { OrgPersonSelect } from '@/components'
export default {
  components: { ReplenishCardForm , OrgPersonSelect},
  props: {
    active: Number,
    queryId: String
  },
  ...
  methods: {
    ...
    validateForm (param) {  //表单校验
      ...
      if(this.form.replenishCardTime == ''){
        this.$toast("补卡时间不能为空");
        return false;
      }
      if(this.form.replenishCardType === ''){
        this.$toast("补卡类型不能为空");
        return false;
      }
      if(this.form.replenishCardReason == ''){
        this.$toast("补卡理由不能为空");
        return false;
      }
    ...
    onSave () {  // 保存业务单
      ...
      this.$post(
        this.queryId ? REPLENISH_CARD_UPDATE : REPLENISH_CARD_CREATE,
        this.form,
        data => {
          this.loading = false
          this.$toast.success(data.msg)
    ...
    onSubmit (id) {   // 启动流程
      ...
      let param = {
        ztReplenishCard: this.form,
        startUserId: this.user.id,
        url: `${window.location.origin + window.location.pathname}/#/workflow?type=replenishCard`,
        multiAssignees: this.assigneesForm.id
      }
      this.$post(
        REPLENISH_CARD_SUBMIT,
        param,
        data => {
          this.loading = false
          this.$toast.success(data.msg)
