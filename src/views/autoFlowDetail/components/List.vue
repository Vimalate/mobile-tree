    ...
    <ShList :api="listApi" :param="form" ref="list" >
      <div class="lists_content" @click="registrationFun(scope.item)" slot-scope="scope">
        <div class="radiuces">
          <div class="flexs">
          <!-- 2 显示审核未通过 -->
          <van-cell :title-style="{'width': '200px'}" :border='false' :title="`${scope.item.name || scope.item.registerName }的${formName}`" />
          <div class="btnIng">
            <div class="defalults listBtns" :style="{color: approColor[scope.item.approStatus] }">{{ approStatus[scope.item.approStatus] }}</div>
          </div>
        </div>
        <div>
          <div class="btns">
            ...
            <div class="boxed">
              <div class="lefts">{{ scope.item.createTime }}</div>
...
<script>
import { AUTOTABLE_FORM_LIST } from '@/apis/autoflow'
export default {
  props: {
    active: Number
  },
  ...
  data () {
    return {
      listApi: AUTOTABLE_FORM_LIST + this.$route.query.id,
      formId: this.$route.query.formId,
      formName: this.$route.query.name,
      form: {},
      approStatus: ['保存', '提交', '审核中', '转审', '同意', '拒绝', '作废',  '结束'],
      approColor: ['#FF9900', '#FF9900', '#FF9900', '#FF9900', '#00A950', '#F26649', '#F26649', '#F26649']
    }
  },
  methods: {
    registrationFun (item) {
      if (item.approStatus == 0) {
        ...
        this.$router.push({path: '/autoflowAppro', query:{id: item.id, formId: this.formId}})
      }
    },
    addLink () {
      this.$emit('update:active', 0)
